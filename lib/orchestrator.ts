...
export async function processPdf(env: Env, jobId: string, pdfBytes: Buffer): Promise<void> {
  ...
  // Stage 1: Upload to Lyzr WITH VLM parse_config baked into the URL query string.
  // Single hop — no separate VLM-parse stage; lyzr returns a multi-page-readable
  // asset_id directly. This replaces the earlier wrapper service round-trip.
  let assetId: string;
  try {
    updateStage(jobId, "upload", { status: "running", started_at: Date.now() });
    assetId = await uploadToLyzr(env, pdfBytes, fileName);
    updateStage(jobId, "upload", { status: "done", ended_at: Date.now(), asset_id: assetId });
  } catch (err) {
  ...
  // Stage 2: Three agents IN PARALLEL via Promise.allSettled
